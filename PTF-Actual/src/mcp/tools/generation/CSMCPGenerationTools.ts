...
const generateTestDataFileTool = defineTool()
    ...
    .description('Generate a JSON test data file. Pass actual records for real data, or field definitions for synthetic generation.')
    .category('generation')
 ...
 */
function buildGlobalOverrideTemplate(
    project: string,
    firstEnv: string = 'sit',
    module?: string,
    crossDomainEnabled: boolean = true,
): string {
    const moduleSeg = module ? `/${module}` : '';
    return `# ============================================================================
...
# Project and environment settings. ENVIRONMENT defaults to the first env
# discovered during analysis (or 'sit' if none); override via env var or CLI.
PROJECT=${project}
ENVIRONMENT=${firstEnv}
# BASE_URL is set per-environment in environments/<env>.env
...
# Enable parallel execution. MAX_PARALLEL_WORKERS is the canonical key.
PARALLEL=false
...
# Feature file paths. Module sub-folders are picked up automatically by the **/**.feature glob.
FEATURES=test/${project}/features/**/*.feature
FEATURE_PATH=test/${project}/features${moduleSeg}

# Test retry configuration
RETRY_COUNT=2

# Step definition paths
STEP_DEFINITIONS_PATH=test/${project}/steps${moduleSeg};test/${project}/steps;test/common/steps

# Test data path
TEST_DATA_PATH=test/${project}/data${moduleSeg};test/${project}/data
...
# Default enabled — legacy enterprise apps almost always have an SSO redirect.
# Set to false if your app is single-domain.
CROSS_DOMAIN_NAVIGATION_ENABLED=${crossDomainEnabled}
...
# Cross-domain navigation (enable for SSO/Netscaler/IDP-based apps).
# Defaulted from analysis — legacy enterprise apps almost always have SSO.
CROSS_DOMAIN_NAVIGATION_ENABLED=${crossDomainEnabled}

# ============================================================================
# Credentials
# ============================================================================
# Replace with real values. Use \`npx cs-playwright-mcp encrypt <value>\` to
# generate the ENCRYPTED:... form for sensitive fields. Leaving these blank
# will cause login steps to fail with a clear "credential not set" error.
DEFAULT_USERNAME=<paste-username-or-leave-blank-to-prompt>
DEFAULT_PASSWORD=ENCRYPTED:<paste-encrypted-password-here-or-use-encrypt-cli>
...
# ====================================================================================
# ENVIRONMENT CREDENTIALS (override common/common.env defaults)
# ====================================================================================
# ${extractedUsername || extractedPassword
    ? `Extracted from legacy resources/${env}/env.properties:`
    : `Fill these in OR keep defaults from common.env. Use \`npx cs-playwright-mcp encrypt <value>\` for ENCRYPTED:... form.`}
${extractedUsername !== undefined ? `DEFAULT_USERNAME=${extractedUsername}` : '# DEFAULT_USERNAME='}
${extractedPassword !== undefined ? `DEFAULT_PASSWORD=${extractedPassword.startsWith('ENCRYPTED:') ? extractedPassword : `ENCRYPTED:${extractedPassword}`}` : '# DEFAULT_PASSWORD=ENCRYPTED:'}
...
const generateConfigScaffoldTool = defineTool()
    ...
    .description('Generate config directory: global.env, common/common.env, environments/{env}.env. Appends missing properties if files exist (safe to re-run).')
    .category('generation')
    ...
    .stringParam('module', 'Module name (optional sub-folder under test/<project>/.../)')
    ...
    .objectParam('envBaseUrls', 'Per-env BASE_URL overrides extracted from legacy properties files, e.g. { sit: "https://app-sit.real.com" }. Wins over baseUrl template.', undefined)
    .objectParam('envCredentials', 'Per-env credentials extracted from legacy properties files, e.g. { sit: { username: "u", password: "p" } }', undefined)
    ...
    .booleanParam('crossDomainEnabled', 'Default for CROSS_DOMAIN_NAVIGATION_ENABLED (default true — enterprise SSO is the norm)')
    ...
    .handler(async (params, context) => {
        ...
        const crossDomainEnabled = params.crossDomainEnabled !== false; // default true
        const envBaseUrls = (params.envBaseUrls as Record<string, string>) || {};
        const envCredentials = (params.envCredentials as Record<string, { username?: string; password?: string }>) || {};
        const firstEnv = environments[0] || 'sit';
        ...
        const globalContent = buildGlobalOverrideTemplate(project, firstEnv, moduleName, crossDomainEnabled);
        ...
        const commonContent = buildCommonTemplate(
            ...
            dbAliases, apiTesting, adoIntegration, moduleName, crossDomainEnabled,
        );
        ...
        // If envBaseUrls / envCredentials are provided (typically from an
        // analyze-phase extraction of legacy resources/<env>/env.properties),
        // wire those real values in. Otherwise fall back to the placeholder
        // template URL.
        for (const env of environments) {
            const resolvedBase = envBaseUrls[env] || baseUrlTemplate;
            const envCreds = envCredentials[env] || {};
            const envContent = buildEnvironmentTemplate(
                project, env, resolvedBase, dbAliases, apiTesting, apiBaseUrl, adoIntegration,
                envCreds.username, envCreds.password,
            );
...
const generateDbQueriesConfigTool = defineTool()
    ...
    .handler(async (params, context) => {
        ...
        if (queries.length === 0) {
            // Stub file — emit a commented example so the operator knows the
            // shape. Skill: named-query-env-entry.
            content += `# ----------------------------------------------------------------------------\n`;
            content += `# EXAMPLE (uncomment + edit to add your first query):\n`;
            content += `# ----------------------------------------------------------------------------\n`;
            content += `# Fetch a user record by external id\n`;
            content += `# DB_QUERY_GET_USER_BY_EXT_ID=SELECT * FROM users WHERE external_id = ?\n`;
            content += `\n`;
            content += `# Rules:\n`;
            content += `#   - Key MUST start with DB_QUERY_ followed by SHOUTING_SNAKE_CASE.\n`;
            content += `#   - Value is the SQL with ? placeholders for parameters.\n`;
            content += `#   - One query per line (no multi-line SQL — use semicolons inside a single line).\n`;
            content += `#   - Comment above each query describes intent + expected columns.\n`;
            content += `# Skill reference: .github/skills/named-query-env-entry/SKILL.md\n`;
        }
